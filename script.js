...
fetch(endpoint, {
    ...
    function openTest(e) {
      ...
      function createQuestion(firstQ) {
        ...
        for (
          var g = 0;
          g < questionss[id_section][test_type][test_index][firstQ].length - 1;
          g++
        ) {
          ...
          var answerText = document.createTextNode(
            questionss[id_section][test_type][test_index][firstQ][g + 1]
          );
        ...
        for (var m = answerList.length - 1; m > 0; m--) {
          ...
          [answerList[m], answerList[newIndex]] = [
            answerList[newIndex],
            answerList[m],
          ];
        }
        ...
        answerList.forEach((ans) => {
          if (test_type !== "truefalse") {
            var optionAnswer =
              sortingIndex[answerList.indexOf(ans)] +
              " " +
      ...
      for (
        var q = 0;
        q < questionss[id_section][test_type][test_index].length;
        q++
      ) {
        createQuestion(q);
      }
    ...
    setTimeout(() => {
      ...
      if (document.querySelectorAll(".goog-te-combo").length > 1) {
        document.getElementsByClassName("seconnT")[0].style.display =
          "inline-block";
      }
    }, 1000);
        ...
        input.addEventListener("change", () => {
          if (
            !document
              ...
              .classList.contains("activ_session")
          ) {
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        if (questions[i][1] !== correctAnswer) {
          ...
          if (correctIndex !== -1) {
            ...
            [questions[i][1], questions[i][correctIndex]] = [
    ...
    const fixedQuestions = ensureCorrectAnswersAtIndex1(
      questionData,
      correctAnswers
    );
